...
const OperationMoneyServiceHealthCheck = "/fastcampus.money.v1.MoneyService/HealthCheck"
...
func RegisterMoneyServiceHTTPServer(s *http.Server, srv MoneyServiceHTTPServer) {
	r := s.Route("/")
	r.GET("/v1/health", _MoneyService_HealthCheck0_HTTP_Handler(srv))
...
func _MoneyService_HealthCheck0_HTTP_Handler(srv MoneyServiceHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		var in emptypb.Empty
		if err := ctx.BindQuery(&in); err != nil {
			return err
		}
		http.SetOperation(ctx, OperationMoneyServiceHealthCheck)
...
func (c *MoneyServiceHTTPClientImpl) HealthCheck(ctx context.Context, in *emptypb.Empty, opts ...http.CallOption) (*HealthResponse, error) {
	var out HealthResponse
	pattern := "/v1/health"
	path := binding.EncodeURL(pattern, in, true)
	opts = append(opts, http.Operation(OperationMoneyServiceHealthCheck))
	opts = append(opts, http.PathTemplate(pattern))
	err := c.cc.Invoke(ctx, "GET", path, nil, &out, opts...)
	if err != nil {
		return nil, err
