...
const OperationMembershipServiceHealthCheck = "/fastcampus.membership.v1.MembershipService/HealthCheck"
...
func RegisterMembershipServiceHTTPServer(s *http.Server, srv MembershipServiceHTTPServer) {
	r := s.Route("/")
	r.GET("/v1/health", _MembershipService_HealthCheck0_HTTP_Handler(srv))
...
func _MembershipService_HealthCheck0_HTTP_Handler(srv MembershipServiceHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		var in emptypb.Empty
		if err := ctx.BindQuery(&in); err != nil {
			return err
		}
		http.SetOperation(ctx, OperationMembershipServiceHealthCheck)
...
type MembershipServiceHTTPClient interface {
	...
	HealthCheck(ctx context.Context, req *emptypb.Empty, opts ...http.CallOption) (rsp *HealthResponse, err error)
...
func (c *MembershipServiceHTTPClientImpl) HealthCheck(ctx context.Context, in *emptypb.Empty, opts ...http.CallOption) (*HealthResponse, error) {
	var out HealthResponse
	pattern := "/v1/health"
	path := binding.EncodeURL(pattern, in, true)
	opts = append(opts, http.Operation(OperationMembershipServiceHealthCheck))
	opts = append(opts, http.PathTemplate(pattern))
	err := c.cc.Invoke(ctx, "GET", path, nil, &out, opts...)
	if err != nil {
		return nil, err
